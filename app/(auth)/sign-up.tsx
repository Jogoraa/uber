...
const CustomerSignUp = () => {
  ...
  const onSignUpPress = async () => {
    ...
    try {
      ...
      const message = err?.errors?.[0]?.longMessage || "An error occurred";
  ...
  const onPressVerify = async () => {
    ...
    try {
      ...
      const message = err?.errors?.[0]?.longMessage || "Verification error";
    ...
    <ScrollView className="flex-1 bg-white">
      <View className="flex-1 bg-white">
        ...
        <ReactNativeModal
          ...
          onModalHide={() => verification.state === "success" && setShowSuccessModal(true)}
        >
